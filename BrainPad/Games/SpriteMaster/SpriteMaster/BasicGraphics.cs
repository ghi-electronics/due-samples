    ...
    internal class BasicGraphics {
        ...
        public void DrawRectangle(uint color, int x, int y, int width, int height) {
            ...
            for (var i = x; i < x + width; i++) {
                this.SetPixel(color, i, y);
                this.SetPixel(color, i, y + height - 1);
            }

            for (var i = y; i < y + height; i++) {
                this.SetPixel(color, x, i);
                this.SetPixel(color, x + width - 1, i);
            }
        }
        ...
        public void DrawCircle(uint color, int x, int y, int radius) {
            ...
            this.SetPixel(color, centerX, centerY + radius);
            this.SetPixel(color, centerX, centerY - radius);
            this.SetPixel(color, centerX + radius, centerY);
            this.SetPixel(color, centerX - radius, centerY);
            ...
            while (dX < dY) {
                ...
                this.SetPixel(color, centerX + dX, centerY + dY);
                this.SetPixel(color, centerX - dX, centerY + dY);
                this.SetPixel(color, centerX + dX, centerY - dY);
                this.SetPixel(color, centerX - dX, centerY - dY);

                this.SetPixel(color, centerX + dY, centerY + dX);
                this.SetPixel(color, centerX - dY, centerY + dX);
                this.SetPixel(color, centerX + dY, centerY - dX);
                this.SetPixel(color, centerX - dY, centerY - dX);
            }
            ...
            for (var col = 0; col < 5; col++) {
                ...
                for (var row = 0; row < 5; row++) {
                    if ((fontCol & (1 << (4 - row))) != 0)
                        this.SetPixel(color, x + row, y + col);
                    else
                        if (clear)
                        this.SetPixel(0, x + row, y + col);
                        ...
                        for (var verticleFontSize = 0; verticleFontSize < 8; verticleFontSize++) {
                            for (var vs = 0; vs < vScale; vs++) {
                                if ((this.mono8x5[index + horizontalFontSize] & (1 << verticleFontSize)) != 0)
                                    this.SetPixel(color, x + (horizontalFontSize * hScale) + hs, y + (verticleFontSize * vScale) + vs);
                            }
                        }
                    }
                }
